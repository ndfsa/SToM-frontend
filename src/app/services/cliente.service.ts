...
export class ClienteService {
  ...
  modifyCliente(cliente){
    let url = 'http://localhost:8083/SToM/cliente/PUT/'+ this.globals.global_id_cliente;
    let data = JSON.stringify(cliente);
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    headers.append('Accept', 'application/json');
  ...
  deleteCliente(){
    let url = 'http://localhost:8083/SToM/cliente/borrar/'+ this.globals.global_id_cliente;
